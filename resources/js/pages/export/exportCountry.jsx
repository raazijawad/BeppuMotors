...
export default function ExportCountry({ country = null, customers = [] }) {
    ...
    const handleAddCustomer = (e) => {
        ...
        router.post(
            ...
            { name },
            {
                ...
                onError: (errors) => {
                    setError(errors.name ?? 'Failed to add customer.');
                },
            },
        );
    };
    ...
    return (
        <div className="flex h-screen flex-col overflow-hidden">
            <Head title={country?.name ?? 'Export'} />
                    ...
                    {customers.length === 0 ? (
                        ...
                        <div className="grid w-full grid-cols-2 gap-4 gap-y-8 md:grid-cols-6 md:gap-4">
                            {customers.map((customer) => (
                                <div
                                    key={customer.id}
                                    className="flex h-44 items-center justify-center rounded-lg border border-[#19140035] bg-white px-2 text-center shadow-sm hover:shadow-md md:h-28 dark:border-[#3E3E3A] dark:bg-[#161615]"
                                >
                                    <span className="text-sm font-medium break-words text-[#706f6c] dark:text-[#A1A09A]">
                                        {customer.name}
                                    </span>
                                </div>
                            ))}
